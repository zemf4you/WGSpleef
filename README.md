# WGSpleef
## RU
Плагин на сплиф для майнкрафта, который работает с WorldGuard. Заточен на сервера с выживанием и без BungeeCord.
### Игровой процесс
...
arenas:
  spleef:  # Название защищённого региона WorldGuard. Количество арен неограничено
    ...
    startCd: 10  # Обратной отсчёт после того, как набралось минимальное количество игроков на арене (в секундах)
...
#### lang.yml
В данном примере используется русский язык. В названии конфига следует указывать используемый язык (в конкретном случае файл надо назвать **ru.yml**)
```yaml
help:
  admin: |-
